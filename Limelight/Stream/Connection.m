...
int DrDecoderSetup(int videoFormat, int width, int height, int redrawRate,
...
void DrStart(void) { [renderer start]; }

void DrStop(void) { [renderer stop]; }
...
int DrSubmitDecodeUnit(PDECODE_UNIT decodeUnit) {
  ...
  if (!lastFrameNumber) {
    ...
    // Any frame number greater than m_LastFrameNumber + 1 represents a dropped
    // frame
    currentVideoStats.networkDroppedFrames +=
        decodeUnit->frameNumber - (lastFrameNumber + 1);
  ...
  if (decodeUnit->frameHostProcessingLatency != 0) {
    ...
    if (decodeUnit->frameHostProcessingLatency >
        currentVideoStats.maxHostProcessingLatency) {
      currentVideoStats.maxHostProcessingLatency =
          decodeUnit->frameHostProcessingLatency;
    }

    currentVideoStats.framesWithHostProcessingLatency++;
    currentVideoStats.totalHostProcessingLatency +=
        decodeUnit->frameHostProcessingLatency;
  }
  ...
  while (entry != NULL) {
    // Submit parameter set NALUs directly since no copy is required by the
    // decoder
...
int ArInit(int audioConfiguration, POPUS_MULTISTREAM_CONFIGURATION opusConfig,
  ...
  audioFrameSize =
      opusConfig->samplesPerFrame * sizeof(short) * opusConfig->channelCount;
  ...
  // Disable lowering volume of other audio streams (SDL sets
  // AVAudioSessionCategoryOptionDuckOthers by default)
...
void ArDecodeAndPlaySample(char *sampleData, int sampleLength) {
  ...
  decodeLen = opus_multistream_decode(opusDecoder, (unsigned char *)sampleData,
                                      sampleLength, (short *)audioBuffer,
                                      audioConfig.samplesPerFrame, 0);
  if (decodeLen > 0) {
    // Provide backpressure on the queue to ensure too many frames don't build
    // up in SDL's audio queue.
    while (SDL_GetQueuedAudioSize(audioDevice) / audioFrameSize > 10) {
      SDL_Delay(1);
    }

    if (SDL_QueueAudio(audioDevice, audioBuffer,
                       sizeof(short) * decodeLen * audioConfig.channelCount) <
        0) {
...
void ClStageFailed(int stage, int errorCode) {
  [_callbacks stageFailed:LiGetStageName(stage)
                withError:errorCode
            portTestFlags:LiGetPortFlagsFromStage(stage)];
}

void ClConnectionStarted(void) { [_callbacks connectionStarted]; }
...
void ClRumble(unsigned short controllerNumber, unsigned short lowFreqMotor,
              unsigned short highFreqMotor) {
  [_callbacks rumble:controllerNumber
...
void ClRumbleTriggers(uint16_t controllerNumber, uint16_t leftTriggerMotor,
                      uint16_t rightTriggerMotor) {
  [_callbacks rumbleTriggers:controllerNumber
                 leftTrigger:leftTriggerMotor
                rightTrigger:rightTriggerMotor];
}

void ClSetMotionEventState(uint16_t controllerNumber, uint8_t motionType,
                           uint16_t reportRateHz) {
  [_callbacks setMotionEventState:controllerNumber
                       motionType:motionType
                     reportRateHz:reportRateHz];
}

void ClSetControllerLED(uint16_t controllerNumber, uint8_t r, uint8_t g,
                        uint8_t b) {
...
- (id)initWithConfig:(StreamConfiguration *)config
  ...
  strncpy(_hostString, [rawAddress cStringUsingEncoding:NSUTF8StringEncoding],
  ...
  if ([Utils isActiveNetworkVPN]) {
    ...
    // Detect remote streaming automatically based on the IP address of the
    // target
    _streamConfig.streamingRemotely = STREAM_CFG_AUTO;
    _streamConfig.packetSize = 1392;
  }

  memcpy(_streamConfig.remoteInputAesKey, [config.riKey bytes],
         [config.riKey length]);
...
- (void)main {
  [initLock lock];
  LiStartConnection(&_serverInfo, &_streamConfig, &_clCallbacks, &_drCallbacks,
                    &_arCallbacks, NULL, 0, NULL, 0);
  [initLock unlock];
}
